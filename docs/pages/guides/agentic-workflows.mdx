...
| `maxTokens` | number | Token limit for generation (optional) |
| `temperature` | number | Sampling temperature (optional) |
| `model` | string | Registered model to handle generation |
| `enableThinking` | boolean | Enable thinking mode where supported (optional) |
| `toolChoice` | string[] | Available tools for this step (optional) |
| `maxAttempts` | number | Retry attempts on failure (optional) |

## Memory Management

Workflows automatically manage conversation history. Configure memory settings when calling `runWorkflow()`:
...
const memoryConfig = {
  ...
  compressionThreshold: 0.8,    // Compress at 80% capacity
  memoryCompressorConfig: {
    name: 'sliding-window'      // Compression strategy
  }
};

for await (const iteration of agent.runWorkflow(prompt, workflow, memoryConfig)) {
  // ...
}
```
